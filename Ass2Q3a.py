...
wx = np.where(abs(dxdt) < 0.001)[0] #Stores the indices of dxdt and dydt that are approximately 0 (close to -0 and +0 by using the absolute value function)
wy = np.where(abs(dydt) < 0.001)[0] #the np.where function returns a tuple, but since we only need the first column, we access it using the index of 0 in where(..)[0]
